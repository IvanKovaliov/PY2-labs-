...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    AppendMenu(hSubMenu, MF_STRING, ID_LAB9, L"Лаб. 9: DLL");
...
BOOL CALLBACK Lab9DlgProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    static SumArrayFunc SumArray = NULL;
    ...
    case WM_INITDIALOG:
        ...
        SetDlgItemText(hwndDlg, IDC_INPUT, L"");
        SetDlgItemText(hwndDlg, IDC_OUTPUT, L"");
        hLib = LoadLibrary(L"ArrayLib.dll");
        if (hLib) {
            SumArray = (SumArrayFunc)GetProcAddress(hLib, "SumArray");
            if (!SumArray) {
                MessageBox(hwndDlg, L"Не удалось загрузить функцию SumArray!", L"Ошибка", MB_OK | MB_ICONERROR);
                FreeLibrary(hLib);
                hLib = NULL;
            }
        } else {
            MessageBox(hwndDlg, L"Не удалось загрузить ArrayLib.dll!", L"Ошибка", MB_OK | MB_ICONERROR);
        }
        return TRUE;
    case WM_COMMAND:
        switch (LOWORD(wParam)) {
